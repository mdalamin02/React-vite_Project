import React from "react";

const Check = () => {
  return (
    <>
      <svg
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <rect width="24" height="24" rx="8" fill="none" />
        <path
          d="M9.7 11.3C9.3 10.9 8.7 10.9 8.3 11.3C7.9 11.7 7.9 12.3 8.3 12.7L11.3 15.7C11.5 15.9 11.7 16 12 16C12.3 16 12.5 15.9 12.7 15.7L19.7 7.70003C20 7.20003 20 6.60003 19.5 6.30003C19.1 6.00003 18.5 6.00003 18.2 6.40003L12 13.5L9.7 11.3Z"
          fill="#00A88E"
        />
        <path
          d="M21 11C20.4 11 20 11.4 20 12C20 16.4 16.4 20 12 20C7.6 20 4 16.4 4 12C4 9.9 4.8 7.9 6.3 6.4C7.8 4.8 9.8 4 12 4C12.6 4 13.3 4.1 13.9 4.2C14.4 4.4 15 4.1 15.2 3.5C15.4 2.9 15 2.5 14.5 2.3H14.4C13.6 2.1 12.8 2 12 2C6.5 2 2 6.5 2 12.1C2 14.7 3.1 17.3 4.9 19.1C6.8 21 9.3 22 11.9 22C17.4 22 21.9 17.5 21.9 12C22 11.4 21.5 11 21 11Z"
          fill="#00A88E"
        />
      </svg>
    </>
  );
};

export default Check;
